...
/// Receiver-side USM: informs, discovery Reports, and responses as the authoritative engine.
/// TEDGE-DOT-PATCH(5)
pub mod receiver;
pub use receiver::{
    Header, LocalEngine, Message, Outgoing, Refusal, SecurityLevel, UsmStats, encode,
    receive_non_authoritative,
};
...
pub enum AuthErrorKind {
    ...
    /// The message asks for a security level the user has no keys for. TEDGE-DOT-PATCH(5)
    UnsupportedSecLevel,
}
...
impl fmt::Display for AuthErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AuthErrorKind::UnsupportedSecLevel => write!(f, "Unsupported security level"),
        }
    }
}
...
impl Security {
    ...
    /// The authoritative engine's time now, as estimated from the last value learned plus the
    /// local time elapsed since. TEDGE-DOT-PATCH(6): upstream sent the time learned at
    /// discovery (or from the last response) unchanged, so a request more than 150 s after the
    /// previous one fell outside the agent's time window and failed.
    pub(crate) fn send_engine_time(&self) -> i64 {
        let state = &self.authoritative_state;
        if state.engine_boots == 0 {
            return state.engine_time;
        }
        let elapsed = i64::try_from(state.start_time.elapsed().as_secs()).unwrap_or(i64::MAX);
        state
            .engine_time
    ...
    /// The security level this user is configured for. TEDGE-DOT-PATCH(5)
    pub fn security_level(&self) -> SecurityLevel {
        match self.auth {
            Auth::NoAuthNoPriv => SecurityLevel::NoAuthNoPriv,
            Auth::AuthNoPriv => SecurityLevel::AuthNoPriv,
            Auth::AuthPriv { .. } => SecurityLevel::AuthPriv,
        }
    }
    ...
    fn encrypt_des(&self, data: &[u8], engine_boots: i64) -> Result<(Vec<u8>, Vec<u8>)> {
        let mut salt = [0; 8];
        salt[..4].copy_from_slice(&u32::try_from(engine_boots)?.to_be_bytes());
        #[cfg(feature = "crypto-openssl")]
    ...
    fn encrypt_aes(
        ...
        iv.extend_from_slice(&u32::try_from(engine_boots)?.to_be_bytes());
        iv.extend_from_slice(&u32::try_from(engine_time)?.to_be_bytes());
        let salt_pos = iv.len();
        iv.resize(iv_len, 0);
    ...
    pub(crate) fn encrypt(&self, data: &[u8]) -> Result<(Vec<u8>, Vec<u8>)> {
        self.encrypt_with(data, self.engine_boots(), self.send_engine_time())
    }

    /// Encrypt for a message carrying these msgAuthoritativeEngineBoots/Time, which the AES IV
    /// is built from. TEDGE-DOT-PATCH(5)
    pub(crate) fn encrypt_with(
        ...
        match cipher_kind {
            Cipher::Des => self.encrypt_des(data, engine_boots),
            Cipher::Aes128 => self.encrypt_aes(data, 16, engine_boots, engine_time),
            Cipher::Aes192 => self.encrypt_aes(data, 24, engine_boots, engine_time),
            Cipher::Aes256 => self.encrypt_aes(data, 32, engine_boots, engine_time),
        }
    }
    ...
    fn decrypt_des(&mut self, encrypted: &[u8], priv_params: &[u8]) -> Result<()> {
        ...
        #[cfg(all(feature = "crypto-rust", not(feature = "crypto-openssl")))]
        {
            ...
            // TEDGE-DOT-PATCH(5): the padding is not PKCS#7. RFC 3414 §8.1.1.2 leaves its
            // octets unspecified and the receiver ignores them (the scoped PDU carries its own
            // length); net-snmp pads with zeros, which upstream rejected as "Invalid padding".
            Ok(())
        }
    }
    ...
    /// Decrypt a message that carried these msgAuthoritativeEngineBoots/Time (the AES IV).
    /// TEDGE-DOT-PATCH(5)
    pub(crate) fn decrypt_with(
        ...
        match cipher_kind {
            ...
            Cipher::Aes128 => {
                self.decrypt_aes(encrypted, priv_params, 16, engine_boots, engine_time)
            }
            Cipher::Aes192 => {
                self.decrypt_aes(encrypted, priv_params, 24, engine_boots, engine_time)
            }
            Cipher::Aes256 => {
                self.decrypt_aes(encrypted, priv_params, 32, engine_boots, engine_time)
            }
        }
    }
}
...
impl<'a> Pdu<'a> {
    ...
    pub(crate) fn parse_v3(
        ...
        if flags & V3_MSG_FLAGS_AUTH == 0 {
            ...
            if security.need_auth() {
                // TEDGE-DOT-PATCH(3): upstream zeroed the authentication parameters in place,
                // writing through a pointer derived from the shared `&[u8]` input (undefined
                // behaviour; a crash for input in read-only memory). Sign a copy instead.
...
pub(crate) fn build(
    ...
    buf.push_sequence(|buf| {
        ...
        sec_buf_seq.push_sequence(|buf| {
            ...
            buf.push_integer(security.send_engine_time()); // time (TEDGE-DOT-PATCH(6))
